	...
	$(document).ready(function(){
		...
		function set_search_field(id){
			...
			$(id + " .add-input-group").on('click', function(e){
				...
				if( typeof $(this).data("html") != typeof undefined ){
					...
					$(target + " .search-input-group").append($block);				
				}
				else{
					...
					$(this).next(".search-input-group").append($clone);
			...
			$(id + " .search-input-group").on('click', ".remove-input-group", function(e){
				...
				typing = setTimeout(function() {
					...
					$.ajax({
						...
						data : {
							...
							action 		: "render_term_suggestions",
							taxonomy 	: taxonomy,
							h			: hierarchical,
							o			: operator,
							s 			: query,
		...
		function set_task_field(id){
			...
			$(document).on("click", id + " .autocomplete-items div", function() {
				...
				let index=$(this).index()
				let data=_tasks_input_suggestions_data[index];
				let data_holder = $(this).parents().eq(4).find(id + " .data")
				let name = $(id + " .data input:first").attr("name");
			...
			$(id + " input").on( "keyup", function(event) {
                ...
                var task_type = $(id).attr("data-type");
                
                var item_type = $('#bulk-editor-filters select:first').val();
                
                var task_num = $(id + " .data .item").length + 1;
				...
				typing = setTimeout(function() {
					...
					$.ajax({
						...
						data : {
							...
							action 	: "render_task_suggestions",
                            tt      : task_type,
                            it      : item_type,
                            tn      : task_num,
							id		: id,
						...
						if( query == val ){
							...
							_tasks_input_suggestions_data = data;
		...
		let _tasks_input_suggestions_data = null;
		
		$(".tasks-input").each(function(e){
        ...
        function load_task_filters(){
			...
			selectingFilters = setTimeout(function() {
				...
				$.ajax({
					...
					$("#rewbe_task_filters").find('.search-input').each(function(e){
		...
		function load_action_fields(){
			...
			selectingAction = setTimeout(function() {
				...
				$.ajax({
                    ...
                    $("#rewbe_action_fields").find('.tasks-input').each(function(e){
